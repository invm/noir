...
fn convert_value(value: ValueRef) -> serde_json::Value {
    match value.data_type() {
        ...
        rusqlite::types::Type::Text => json!(value.as_str_or_null().expect("Failed to get text").to_owned()),
...
pub fn row_to_object(row: &Row, column_count: usize) -> serde_json::Value {
    ...
    for idx in 0..column_count {
        let name = row.as_ref().column_name(idx).expect("Failed to get column name").to_string();
